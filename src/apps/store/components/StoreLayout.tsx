...
const StoreLayout: React.FC<StoreLayoutProps> = ({ storeId }) => {
  ...
  const navItems = [
    { path: "/store-app", label: "Dashboard", icon: Store },
    { path: "/store-app/inventory", label: "Inventory", icon: Package },
    { path: "/store-app/orders", label: "Orders", icon: ShoppingCart },
    { path: "/store-app/pricing", label: "Pricing", icon: DollarSign },
    { path: "/store-app/analytics", label: "Analytics", icon: BarChart3 },
  ];

  const isActive = (path: string) => {
    if (path === "/store-app") return location.pathname === path;
    ...
    <div className="flex min-h-screen bg-muted/30">
      ...
      <div className="hidden md:flex w-64 flex-col bg-background border-r">
        <div className="flex items-center h-16 px-6 border-b">
          ...
          <span className="text-lg font-semibold">Store App</span>
        </div>
